        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if (hand_landmarks.landmark[4].x < hand_landmarks.landmark[2].x):
                    pydirectinput.click()
                bbox_coor = mp_hands.bounding_box(hand_landmarks)
                x, y, w, h = int(bbox_coor[0]), int(bbox_coor[1]), int(
                    bbox_coor[2] - bbox_coor[0]), int(bbox_coor[3] - bbox_coor[1])

                # Hiển thị tên hành động trên khung chữ nhật
                action_label = "Your Action"
                cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(image, action_label, (x, y - 10),
